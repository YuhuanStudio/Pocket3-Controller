...
@Suite struct CaptureSampleDiagnosticsTests {
    @Test func nonImageCallbacksAreVisibleEvenWhenNoFrameCanBeDelivered() throws {
        ...
        store.recordH264Decode(success: true, durationSeconds: 0.002)
        store.recordH264Decode(success: false, durationSeconds: 0.004)
        let decoded = store.sampleDiagnostics()
        #expect(decoded.decodedH264FrameCount == 1 && decoded.h264DecodeFailureCount == 1)
        #expect(decoded.h264DecodeTotalMilliseconds == 6 && decoded.h264DecodeMaximumMilliseconds == 4)
