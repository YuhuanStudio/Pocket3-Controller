...
struct BluetoothCameraPropertyQuery {
    ...
    mutating func receive(_ data: Data, characteristic: String,
        ...
        guard binding == result.binding, result.finishedUptime == nil, result.localSubmitted,
              !result.cancelled, !result.connectionChanged, let submitted = result.submittedUptime,
              uptime.isFinite, uptime >= submitted, uptime - submitted <= Self.observationDuration,
              lastAcceptedUptime == nil || uptime >= lastAcceptedUptime!,
              characteristic == "FFF4" || characteristic == "FFF5",
              let frame = try? DUMLCodec.decode(data), frame.source == 0x28, frame.destination == 0x02,
              frame.commandSet == 0x00, frame.commandID == 0x99 else { return }
        ...
        guard !result.propertyReceived, frame.flags == 0,
              let push = try? CameraPropertyCodec.decodePush(from: frame), push.property == result.property else { return }
