...
public struct ObjectDetector {
    ...
    public init(resourcesAt path: String, options: SpecializationOptions = .default) async throws {
    ...
    public nonisolated(nonsending) func warmup(imageCount: Int = 1, parameters: DetectionParameters = .default) async throws {
    ...
    public nonisolated(nonsending) func detect(image: CGImage) async throws -> [DetectedObject] {
        try await detect(image: image, parameters: .default)
    }

    /// Detect objects in `image` — convenience wrapper over the batched API.
    public nonisolated(nonsending) func detect(image: CGImage, parameters: DetectionParameters) async throws -> [DetectedObject] {
    ...
    public nonisolated(nonsending) func detect(images: [CGImage]) async throws -> [[DetectedObject]] {
    ...
    public nonisolated(nonsending) func detect(images: [CGImage], parameters: DetectionParameters) async throws
        -> [[DetectedObject]]
    {
    ...
    #if DEBUG
    /// Exports the exact preprocessed input and raw outputs for comparison with
    /// the original PyTorch weights. This bypasses postprocessing only, not the
    /// image preparation or Core AI execution used by detection.
    public nonisolated(nonsending) func evaluationProbe(image: CGImage, parameters: DetectionParameters = .default) async throws -> DetectionProbe {
        let started = ProcessInfo.processInfo.systemUptime
        guard case .ndArray(let descriptor) = functionDescriptor.inputDescriptor(of: imageInputName) else { throw DetectionRuntimeError.invalidConfiguration("Missing image input") }
        let plan = try Self.planBatch(expectedShape: descriptor.shape, imageCount: 1, parameters: parameters)
        let resolved = descriptor.resolvingDynamicDimensions([1, 3, plan.height, plan.width])
        let input = try buildInputNDArray(images: [image], plan: plan, descriptor: resolved, parameters: parameters)
        let values = flattenAsFloat(input)
        let prepared = ProcessInfo.processInfo.systemUptime
        var outputs = try await function.run(inputs: [imageInputName: input])
        let inferred = ProcessInfo.processInfo.systemUptime
        guard let logits = outputs.remove(logitsOutputName)?.ndArray, let boxes = outputs.remove(boxesOutputName)?.ndArray else { throw DetectionRuntimeError.invalidConfiguration("Missing probe outputs") }
        return DetectionProbe(inputScalarType: String(describing: descriptor.scalarType), preparationSeconds: prepared-started, inferenceSeconds: inferred-prepared, inputShape: [1, 3, plan.height, plan.width], inputFloat32: values.withUnsafeBytes { Data($0) }, logitsShape: logits.shape, logits: flattenAsFloat(logits), boxes: flattenAsFloat(boxes))
    }
    #endif
...
#if DEBUG
public struct DetectionProbe: Codable, Sendable {
    public let inputScalarType: String
    public let preparationSeconds: Double
    public let inferenceSeconds: Double
    public let inputShape: [Int]
    public let inputFloat32: Data
    public let logitsShape: [Int]
    public let logits: [Float]
    public let boxes: [Float]
}
#endif
