...
@main struct Pocket3CLI {
    ...
    static func runMCP() async throws {
        ...
        let tools: [MCP.Tool] = [
            ...
            Tool(name: MCPCameraToolContract.formatInventoryName, description: "List AVFoundation video modes and NV12/UYVY host input paths currently advertised by the selected Pocket 3. It does not start capture or claim each advertised combination delivers frames.", inputSchema: empty, annotations: .init(readOnlyHint: true, openWorldHint: false)),
            Tool(name: MCPCameraToolContract.bodyStatusName, description: "Read cached paired-camera Bluetooth telemetry, including any previously received battery, pose, settings, recording and storage observations. It never initializes Bluetooth, scans, pairs, joins Wi-Fi, sends a setting or starts capture. initialized=false means the App has no explicitly initialized Bluetooth session in this process.", inputSchema: empty, annotations: .init(readOnlyHint: true, openWorldHint: false)),
