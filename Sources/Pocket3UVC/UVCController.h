...
@interface UVCController : NSObject
{
  ...
  BOOL                          _ownsInterfaceOpen;
  BOOL                          _explicitInterfaceOpen;
  uint8_t                       _videoInterfaceIndex;
  BOOL                          _hasCameraTerminalDescriptor;
  // Controls retain this controller; zeroing-weak cache values avoid a cycle.
  NSMapTable                    *_controls;
...
// Complete absolute pan/tilt target, already range-checked by the bound session.
// Does not GET_CUR or round to GET_RES. Only the USB request has a deadline;
// opening/closing the interface and physical motion are outside that deadline.
...
/*!
  ...
  Explicitly open or close this controller's interface. An owned explicit open
  remains open across requests until closed by the caller. Exclusive-access
  borrowing never grants ownership and is cleared after each request without
  closing the other client's interface.
*/
...
/*!
  ...
  the receiver has previously instantiated a still-live control, the cached copy is
  ...
  instantiated, the new control is weakly cached and returned autoreleased.
  Retaining a control keeps its parent controller alive, even if the caller
  releases its own controller reference.
*/
