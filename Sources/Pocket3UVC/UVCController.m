...
/*!
  ...
  This method borrows ioServiceObject for the duration of the call. Its caller
  retains ownership and is responsible for releasing any reference it acquired.
*/
...
// The undecorated ID requests the oldest (100) interface, regardless of the
// pointer's declared type. ControlRequestTO requires 182 or newer; descriptors
// already used below require the explicitly declared 220 interface.
static CFUUIDRef P3UVCInterfaceID(void) { return kIOUSBInterfaceInterfaceID220; }
enum { P3UVCDefaultRequestTimeoutMilliseconds = 1000,
       P3UVCMaximumFastRequestTimeoutMilliseconds = 250 };
    ...
    ioServiceObject:(io_service_t)ioServiceObject
  {
    if ( (self = [self init]) ) {
      ...
      if ( [self findControllerInterfaceForServiceObject:ioServiceObject] ) {
        // UVCControl retains its parent so independently retained controls stay
        // usable. Only cache live controls weakly to avoid retaining each other.
        _controls = [[NSMapTable alloc] initWithKeyOptions:NSPointerFunctionsStrongMemory | NSPointerFunctionsCopyIn
...
//
  ...
  - (BOOL) findControllerInterfaceForServiceObject:(io_service_t)ioServiceObject
  {
    ...
    hrc = (*plugInInterface)->QueryInterface(plugInInterface, CFUUIDGetUUIDBytes(P3UVCInterfaceID()), (LPVOID *)&_controllerInterface);
    IODestroyPlugInInterface(plugInInterface);
    ...
    if (hrc != kIOReturnSuccess) return NO;
      ...
      switch ( descriptorPrefix->bDescriptorSubType ) {
        ...
        case VC_HEADER: {
          ...
          while ( basePtr < endPtr ) {
            if ((uint8_t *)endPtr - (uint8_t *)basePtr < sizeof(UVC_Descriptor_Prefix)) break;
            descriptorPrefix = (UVC_Descriptor_Prefix*)basePtr;
            if (descriptorPrefix->bLength < sizeof(UVC_Descriptor_Prefix) ||
                descriptorPrefix->bLength > (uint8_t *)endPtr - (uint8_t *)basePtr) break;
            if ( descriptorPrefix->bDescriptorType == CS_INTERFACE ) {
              switch ( descriptorPrefix->bDescriptorSubType ) {

                case VC_INPUT_TERMINAL: {
                  if (descriptorPrefix->bLength < sizeof(UVC_VC_Terminal_Header_Descriptor)) break;
                  UVC_VC_Terminal_Header_Descriptor   *terminalHeader = (UVC_VC_Terminal_Header_Descriptor*)basePtr;

                  // Camera terminals have the extended camera descriptor.
                  // Other input terminal types must not supply its layout/ID.
                  if (NSSwapLittleShortToHost(terminalHeader->wTerminalType) != 0x0201 ||
                      !terminalHeader->bTerminalId ||
                      terminalHeader->bControlSize > descriptorPrefix->bLength - sizeof(*terminalHeader)) break;
                  if ( terminalHeader->bControlSize > 0 && !_hasCameraTerminalDescriptor ) {
                    [_unitIds setObject:@(terminalHeader->bTerminalId) forKey:@"UVC_INPUT_TERMINAL_ID"];
                    _terminalControlsAvailable = [[NSData alloc] initWithBytes:&terminalHeader->bmControls[0] length:terminalHeader->bControlSize];
                    _hasCameraTerminalDescriptor = YES;
                  }
                  break;
                }
...
//
  ...
  - (BOOL) sendControlRequest:(IOUSBDevRequest)controlRequest
  {
    IOUSBDevRequestTO timed = {
      .bmRequestType = controlRequest.bmRequestType, .bRequest = controlRequest.bRequest,
      .wValue = controlRequest.wValue, .wIndex = controlRequest.wIndex,
      .wLength = controlRequest.wLength, .pData = controlRequest.pData,
      .noDataTimeout = P3UVCDefaultRequestTimeoutMilliseconds,
      .completionTimeout = P3UVCDefaultRequestTimeoutMilliseconds
    };
    return [self sendTimedControlRequest:timed] == kIOReturnSuccess;
  }

  - (IOReturn) sendTimedControlRequest:(IOUSBDevRequestTO)controlRequest
  {
    if (!controlRequest.pData || !controlRequest.wLength ||
        !controlRequest.noDataTimeout || !controlRequest.completionTimeout) return kIOReturnBadArgument;
    if (!_controllerInterface || !(*_controllerInterface)->ControlRequestTO) return kIOReturnUnsupported;
    if ( ! [self openInterfaceIfNeeded] ) return kIOReturnNotOpen;
    @try {
      IOReturn rc = (*_controllerInterface)->ControlRequestTO(_controllerInterface, 0, &controlRequest);
      if (rc == kIOReturnSuccess && controlRequest.wLenDone != controlRequest.wLength) return kIOReturnUnderrun;
      return rc;
    } @finally {
      // A normal request borrows the interface only for this call. An explicit
      // owned open is the sole reason to retain it across requests. In the
      // ExclusiveAccess case this clears logical state without closing USB.
      if ( ! _explicitInterfaceOpen ) [self setIsInterfaceOpen:NO];
    }
  }
...
@implementation UVCController
  ...
  - (IOReturn) writeRollAbsolute:(int16_t)value requestTimeoutMilliseconds:(UInt32)timeout
  {
    if (!timeout || timeout > P3UVCMaximumFastRequestTimeoutMilliseconds) return kIOReturnBadArgument;
    if (!_hasCameraTerminalDescriptor || [self controlIsNotAvailable:@"roll-abs"]) return kIOReturnUnsupported;
    NSUInteger index = [self controlIndexForString:@"roll-abs"];
    if (index == UVCInvalidControlIndex || index >= sizeof(UVCControllerControls) / sizeof(UVCControllerControls[0]))
      return kIOReturnUnsupported;
    const uvc_control_t *control = &UVCControllerControls[index];
    if (control->selector <= 0 || control->selector > UINT8_MAX || !control->uvcTypeDescription || strcmp(control->uvcTypeDescription, "{S2}") != 0)
      return kIOReturnUnsupported;
    NSNumber *unit = _unitIds[control->unitTypeStr];
    if (!unit || !unit.unsignedIntegerValue || unit.unsignedIntegerValue > UINT8_MAX) return kIOReturnUnsupported;
    uint16_t bits = (uint16_t)value;
    uint8_t bytes[2] = {(uint8_t)bits, (uint8_t)(bits >> 8)};
    IOUSBDevRequestTO request = {
      .bmRequestType = USBmakebmRequestType(kUSBOut, kUSBClass, kUSBInterface),
      .bRequest = UVC_SET_CUR, .wValue = control->selector << 8,
      .wIndex = (unit.unsignedIntValue << 8) | _videoInterfaceIndex,
      .wLength = sizeof(bytes), .pData = bytes,
      .noDataTimeout = timeout, .completionTimeout = timeout
    };
    return [self sendTimedControlRequest:request];
  }

  - (IOReturn) writeZoomAbsolute:(uint16_t)value requestTimeoutMilliseconds:(UInt32)timeout
  {
    if (!timeout || timeout > P3UVCMaximumFastRequestTimeoutMilliseconds) return kIOReturnBadArgument;
    if (!_hasCameraTerminalDescriptor || [self controlIsNotAvailable:@"zoom-abs"]) return kIOReturnUnsupported;
    NSNumber *unit = _unitIds[@"UVC_INPUT_TERMINAL_ID"];
    if (!unit || !unit.unsignedIntegerValue || unit.unsignedIntegerValue > UINT8_MAX) return kIOReturnUnsupported;
    uint8_t bytes[2] = {(uint8_t)value, (uint8_t)(value >> 8)};
    IOUSBDevRequestTO request = {
      .bmRequestType = USBmakebmRequestType(kUSBOut, kUSBClass, kUSBInterface),
      .bRequest = UVC_SET_CUR, .wValue = CT_ZOOM_ABSOLUTE_CONTROL << 8,
      .wIndex = (unit.unsignedIntValue << 8) | _videoInterfaceIndex,
      .wLength = sizeof(bytes), .pData = bytes,
      .noDataTimeout = timeout, .completionTimeout = timeout
    };
  ...
  - (IOReturn) writePanTiltAbsolutePan:(int32_t)pan tilt:(int32_t)tilt requestTimeoutMilliseconds:(UInt32)timeout
  {
    if (!timeout || timeout > P3UVCMaximumFastRequestTimeoutMilliseconds) return kIOReturnBadArgument;
    if (!_hasCameraTerminalDescriptor || [self controlIsNotAvailable:@"pan-tilt-abs"]) return kIOReturnUnsupported;
    NSNumber *unit = _unitIds[@"UVC_INPUT_TERMINAL_ID"];
    if (!unit || unit.unsignedIntegerValue == 0 || unit.unsignedIntegerValue > UINT8_MAX) return kIOReturnUnsupported;
    // Unsigned shifts preserve two's-complement negative targets without signed
    // shift/overflow or alignment assumptions. UVC is explicitly little endian.
    uint8_t bytes[8];
    ...
    IOUSBDevRequestTO request = {
      .bmRequestType = USBmakebmRequestType(kUSBOut, kUSBClass, kUSBInterface),
      .bRequest = UVC_SET_CUR, .wValue = CT_PANTILT_ABSOLUTE_CONTROL << 8,
      .wIndex = (unit.unsignedIntValue << 8) | _videoInterfaceIndex,
      .wLength = sizeof(bytes), .pData = bytes,
      .noDataTimeout = timeout, .completionTimeout = timeout
    };
  ...
  + (NSArray*) controlStrings
  {
    ...
    // Pocket3Bridge: allKeys is autoreleased under MRC. The CLI upstream used
    // one pool; a persistent app must own the cached list beyond that pool.
    dispatch_once(&once, ^{
      sharedControlStrings = [[[self controlMapping] allKeys] copy];
    });
...
//
  ...
  + (id) uvcControllerWithService:(io_service_t)ioService
  {
    ...
    // Initialization borrows this service; the caller keeps its own reference.
...
//
  ...
  + (id) uvcControllerWithLocationId:(UInt32)locationId
  {
    ...
    if ( device ) {
      ...
      // Initialization borrows the matching service; balance its +1 reference
      // here on both successful and failed initialization.
...
//
  ...
  - (BOOL) openInterfaceIfNeeded
  {
    if ( _isInterfaceOpen ) return YES;
    if ( ! _controllerInterface ) return NO;
    IOReturn rc = (*_controllerInterface)->USBInterfaceOpen(_controllerInterface);
    if ( rc == kIOReturnSuccess ) {
      _isInterfaceOpen = YES;
      _ownsInterfaceOpen = YES;
    } else if ( rc == kIOReturnExclusiveAccess ) {
      _isInterfaceOpen = YES;
      _ownsInterfaceOpen = NO;
    } else {
      _isInterfaceOpen = NO;
      _ownsInterfaceOpen = NO;
    }
  ...
  {
    if ( isInterfaceOpen ) {
      _explicitInterfaceOpen = [self openInterfaceIfNeeded] && _ownsInterfaceOpen;
      return;
    }
    _explicitInterfaceOpen = NO;
    if ( ! _isInterfaceOpen ) return;
    if ( ! _ownsInterfaceOpen ) {
      // We did not acquire an open, so there is no matching USB close to send.
      _isInterfaceOpen = NO;
      return;
    }
    IOReturn rc = (*_controllerInterface)->USBInterfaceClose(_controllerInterface);
    if ( rc == kIOReturnSuccess || rc == kIOReturnNotOpen || rc == kIOReturnNoDevice ) {
      _isInterfaceOpen = NO;
      _ownsInterfaceOpen = NO;
    }
    // On an unexpected close failure keep ownership, but not explicit intent.
    // A later request/close or deallocation can still release our own open.
  }
    ...
    if ( ! theControl ) {
      if ( ! [self controlIsNotAvailable:controlName] ) {
        ...
        if ( controlIndex != UVCInvalidControlIndex ) {
          ...
          // The weak cache does not own this value. Preserve normal Cocoa
          // return lifetime while allowing callers to retain it independently.
          [theControl autorelease];
        }
      } else {
        [_controls setObject:[NSNull null] forKey:controlName];
      }
