...
- [x] codec capability唯讀基礎：新增`camcap_video_codec`與`camcap_video_format`的opaque、bounded read-only query，未知欄位不會推論成codec或setter。實機`camcap_video_codec` query完整等候後無payload；`camcap_video_format`尚未送出，因下一輪BLE重連先遇`bluetooth_gatt_timeout`。兩者都不是不支援結論，且未改變相機設定。
...
- [x] metrics-only USB trajectory baseline：新驗證器不輸出照片；right與left各完成24個readback sample／verified Stop／manual cleanup＋pause。probe每次前600ms朝指定方向、後600ms反向後hold，因此最終raw position不作左右或物理角度判斷；這只驗服務scheduler reversal→hold流程，不驗尾移。
