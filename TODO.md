...
- [x] 第30–32批依完整開源專案deep research修正兩個核心方向：一般HEVC選項改走可靠BGRA capture＋Mac VideoToolbox local encoder，不再要求AVFoundation提供hvc1；真機一般connect取得91個hvc1 samples／29.996 encoded fps，pause/reconnect後再取得61個／29.942fps並清理。原生UDP9004搖桿採Pocket 3實測center1024±550，release維持04/01 neutral，FE08/FE09接一般native-control入口。Station-mode foundation依Pocket 3實測07/39(E0)→07/48→10秒settle→07/47→LAN 07/07 identity建立，含credential-free cleanup debt；完整Release gate共1,036項通過（Core872、Intelligence44、Evaluation1、App113、XCTest6）。[研究](research/2026-09-13/POCKET3_REFERENCE_PROJECTS_DEEP_RESEARCH.md)、[HEVC真機](artifacts/hardware-complete-2026-09-13/host-hevc-general-product-lifecycle.json)
- [x] 第33批把station foundation接成App垂直切片：SecureField只在記憶體保留現有LAN credentials；FFF5唯一owner依序提交07/39、07/48、07/47並在FFF4/FFF5核對sequence reply；使用者提供camera LAN IPv4後沿用唯一TCP7001/UDP9004 owner，嚴格解析LAN 07/07 camera SSID並只以SHA-256和BLE階段identity比較。Default route前後必須可觀察且相同，command-ready後同一owner交給現有ContinuousGimbalScheduler／FE08／FE09；stop含重新驗證BLE與credential-free cleanup debt。完整Release gate共1,041項通過（Core875、Intelligence44、Evaluation1、App115、XCTest6）；尚未送station真機命令。
- [x] 第34批沿同一station／direct-UVC主線收斂兩個實作缺口：Station模式只掃目前primary IPv4 `/24`，以最多24並發、每候選不超過0.8秒的bound-interface TCP7001探測篩到最多8台，再逐台用既有TCP7001／UDP9004 owner與LAN `07/07`核對BLE階段SSID digest；明確IP仍可作進階override且跳過掃描。Direct UVC則把既有negotiator收斂成固定GET_MAX PROBE→SET_CUR PROBE→GET_CUR PROBE→SET_CUR COMMIT，COMMIT逐byte回送GET_CUR的26-byte block，錯誤、取消、亂序均fail-stop且不重試。完整Release gate共1,049項通過（Core883、Intelligence44、Evaluation1、App115、XCTest6）；下一步是station真機join與原生雲台物理驗收，direct UVC仍需取得VS owner後才可進bulk read。
- [x] 第35批把研究直接收斂成四個可執行缺口：Direct UVC在既有negotiation admission後新增bounded bulk-IN reader與UVC payload→H.264 AU assembler；Station原生雲台新增near/mid/far連續輸入、neutral、FE08、FE09的有界驗收器；USB完整宣告範圍16案matrix已有真正CameraService/UVC owner executor、Stop／逐案restore／reconnect fence；Mac VideoToolbox HEVC可選明確`.hevc`路徑後輸出ordered Annex-B、VPS/SPS/PPS、byte/duration bound與atomic publish。真機另發現paired後camera-AP credential讀取會timeout，Station已改成pair-only也可用exact selected BLE advertisement name建立暫時digest，仍必須由LAN `07/07`完全匹配才交出owner。Capability graph v2逐項攜帶transport、hardware scope、evidence level、license與固定provenance。完整Release gate共1,073項通過（Core905、Intelligence44、Evaluation1、App117、XCTest6）；Station join、原生雲台物理結果與Direct UVC VS ownership／真實bulk bytes仍需本機執行。
- [x] 第36批將Station控制骨架接成三個同owner產品／驗收垂直切片：原生雲台acceptance已有App IPC／CLI exact-session route；普通Video機內start／stop／單一legal format pair已有一般service、CLI與MCP contract，要求fresh 02/80、cam_video_param、capability、單次send及matching terminal readback；white balance／AF mode／color profile／product showcase已有一般產品writer、status與MCP contract，且每項仍以本機writer evidence獨立admit，成功後再用第二筆transaction恢復captured raw baseline。整合審查修正缺少`await`、錯誤App測試owner及restore readback時間序，完整Release gate共1,086項通過（Core916、Intelligence44、Evaluation1、App119、XCTest6）。目前真機Station仍在pair-only／LAN尚未join，故設定writer維持未解鎖，不能把離線contract當成機身寫入成功。
- [x] 第37批完成本階段收斂：機內錄影與四類native setting使用現有Yun disclosure/card排版接入一般App，只有command-ready、fresh legal/readback及獨立writer admission成立才顯示可操作選項，三語與既有footer均保留。Direct UVC normal-open bridge已真正實作26-byte VS class control、固定SET無response、0x82 bounded bulk-IN、AbortPipe與close fence，成功negotiation才建立同owner/generation H.264 reader；審查修正SET回傳資料會令四步協商必敗及仍聲稱`no_pipe`的錯誤證據。ActiveTrack source audit確認A5/A6/A89 packet shape但缺Pocket 3直橫／鏡像座標證據，因此新增3–8個非對稱觸控點、只擬合8種rotation/mirror、residual門檻與exact-session的校準gate，尚未繞過A6 writer admission。完整Release gate共1,091項通過（Core920、Intelligence44、Evaluation1、App120、XCTest6）；依使用者要求本階段結束後暫停。
...
- [x] 第二十批把四案例capture format matrix接成真正bounded executor（逐案connect／warmup／scalar samples／pause／restore，無fallback／影像保存）；新增Mac VideoToolbox host HEVC encoder core與bounded queue/session fence，明確不宣稱USB wire HEVC；GUI手動控制fake harness覆蓋mouse/key/focus loss/Stop/access takeover/reconnect並與real-time heartbeat同suite序列化。Release gate共935項通過（Core776、Intelligence44、Evaluation1、App108、XCTest6）。
- [x] 第二十一批將H.264 decode移出AVFoundation callback，新增bounded ordered async queue、drop/backpressure/latency與generation/cancel/stop fence；Host HEVC新增fresh BGRA/NV12→copied hvc1 session/sink與reconnect cleanup，provenance固定Mac VideoToolbox；format trial分開codec/decode/realtime三層證據與FPS範圍。Release gate共947項通過（Core788、Intelligence44、Evaluation1、App108、XCTest6）。
...
- [x] H.264 host-output直幅性能：舊同步路徑1080×1920／720×1280僅23.74／14.00fps；新bounded async pipeline以20秒warmup實測711／710個avc1 samples全部decode成BGRA，median分別29.98／29.98fps，steady-window零drop/backpressure且restore成功，兩個30fps案例通過。[結果](artifacts/hardware-complete-2026-09-13/h264-portrait-async.json)
- [x] H.264 host-output 4K30最新整合版效能：第20批同步路徑median 21.25fps；第21/22批改為bounded async decode並由自有queue報drop，再只對H.264關閉AVFoundation late-frame discard。20秒warmup實測713個avc1 samples／713 decode、median30.10fps（29.75–30.19）、零decode failure/drop、零AVF drop/backpressure、clean restore，通過5%門檻。[失敗基線](artifacts/hardware-complete-2026-09-12/capture-format-4k30-h264.json)、[通過](artifacts/hardware-complete-2026-09-12/capture-format-4k30-h264-async-pass.json)
- [ ] 4K60 host capture：NV12未宣告；UYVY/H.264 input加host H.264 output仍0 sample／0 runtime error，不能以4K30標成4K60可用。最新NV12 4K24／25／30在12秒warm-up後分別為23.98／25.00／29.97fps；短5秒窗口仍保留pre-stable 20–22fps failure，不把它改寫成format不支援。需新的可合法交付callback的transport，decoder不在此輪資料路徑上。
...
- [x] 上述AVFoundation負路徑已由產品修正取代：一般HEVC選擇現在維持BGRA preview並自動啟動Mac VideoToolbox local encoder，真機兩個capture session均約30 encoded fps且verified；CapabilityGraph transport已更正為`local`。目前consumer仍是bounded sample evidence，沒有冒稱已提供檔案、虛擬攝影機或網路串流。[結果](artifacts/hardware-complete-2026-09-13/host-hevc-general-product-lifecycle.json)
...
- [ ] 完成最新candidate的直幅方向／完整內容、黑邊、視窗縮放與切換重連，以及尚未覆蓋的格式組合。公開beta1的NV12橫幅子矩陣已通過；早期五種直幅的通過受當次機身方向限制，不涵蓋全部模式。新版唯讀 MCP `camera_format_inventory` 在未選相機但僅一台已接機身時列出16個宣告模式（5直幅、6個4K），保持idle／0 frame；這只改善選擇發現，不把宣告當串流通過。最新機身轉直的1080×1920@30 NV12 scalar edge metrics top/bottom=0、左右約0.2%，沒有letterbox訊號，pause後0 frames；未將此標成完整人工方向驗收。[結果](artifacts/hardware-complete-2026-09-11/mcp-format-inventory.json)
